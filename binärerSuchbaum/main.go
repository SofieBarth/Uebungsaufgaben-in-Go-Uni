...
func main() {
	...
	var ursprungswurzel knoten
	ursprungswurzel = knoten{
		wert:   5,
		links:  nil,
		rechts: nil,
	}
	einfügen(&ursprungswurzel, 6)
	einfügen(&ursprungswurzel, 1)
	einfügen(&ursprungswurzel, 2)
	einfügen(&ursprungswurzel, 8)
	einfügen(&ursprungswurzel, 9)
	//einfügen(&ursprungswurzel, 10)
	//einfügen(&ursprungswurzel, 11)
	//einfügen(&ursprungswurzel, 12)
	baumAusgeben(&ursprungswurzel)
	fmt.Println()
	//fmt.Println(ursprungswurzel)
	//fmt.Println(ursprungswurzel.links)
	//fmt.Println(ursprungswurzel.rechts)
	fmt.Println(höheDesBaums(&ursprungswurzel))
	fmt.Println(kleinsterKnoten(&ursprungswurzel))
	fmt.Println(höheDesWerts(&ursprungswurzel, 9, 1))
	inkrementiereAlleWerte(&ursprungswurzel)
	baumAusgeben(&ursprungswurzel)
}
...
func höheDesBaums(wurzel *knoten) int {
	...
	if wurzel != nil {
		hoeheLinks := höheDesBaums(wurzel.links)
		hoeheRechts := höheDesBaums(wurzel.rechts)
